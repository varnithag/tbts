...
public class maxmin_Scheduler{
	...
	private static void calmakespan(List<Cloudlet> list) {
		...
		Log.printLine("\nMakespan="+(ms+vmandTaskCount.taskscount));
	}
	/**
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS){
				...
				Log.printLine( indent + indent + cloudlet.getResourceId() + indent + indent + indent + cloudlet.getVmId() +
						indent + indent + indent + dft.format((cloudlet.getActualCPUTime()+vmandTaskCount.taskscount)) +
						indent + indent + dft.format(cloudlet.getExecStartTime())+ indent + indent + indent + dft.format((cloudlet.getFinishTime()+vmandTaskCount.taskscount)));
			}
		}
